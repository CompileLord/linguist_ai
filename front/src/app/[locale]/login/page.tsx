  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to load user profile, redirecting to onboarding:", profileErr);
        router.push("/onboarding");
      }
    } catch (err: any) {
      console.error("Login failed:", err);
      const detail = err?.data?.detail || err?.data || err?.error || err?.message || "Login failed";
      ...
      <div className="flex-1 flex items-center justify-center p-6 order-1">
        <main className="w-full max-w-[420px] flex flex-col gap-sm animate-fade-in">
          ...
          <div className="bg-surface rounded-xl border border-outline p-8 w-full flex flex-col gap-6 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
            ...
            <button
              type="button"
              className="w-full bg-surface-raised border border-outline text-on-surface font-medium py-2.5 px-4 rounded-lg hover:bg-[#1E1E24] active:scale-[0.96] transition-[transform,background-color,border-color] duration-150 flex items-center justify-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
            >
