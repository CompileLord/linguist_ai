...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (isInitialized && isAuthenticated && !justRegistered.current) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row text-on-surface bg-background font-body-md antialiased selection:bg-primary/20 selection:text-primary">
      {/* Form Section */}
      <div
        className="flex-1 flex items-center justify-center p-[0_24px] order-1 relative"
        style={{
          background: "#050507",
          backgroundImage:
      ...
      >
        ...
        <main className="w-full max-w-[420px] flex flex-col gap-sm animate-fade-in relative z-10 my-3 backdrop-blur-xs">
          {/* Card Container */}
          <div className="bg-black/25 backdrop-blur-[70px] rounded-xl border border-outline py-4 px-8 w-full flex flex-col gap-3 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex flex-col gap-4">
              <button
                ...
                disabled
                title="Coming soon"
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg bg-surface-raised border border-outline text-on-surface/40 font-medium cursor-not-allowed text-sm focus:outline-none opacity-50"
              >
                <svg
                  className="w-4 h-4"
                  viewBox="0 0 24 24"
              ...
              >
                ...
                <div className="flex flex-col gap-1.5">
                  <label
                    className="font-label-md text-xs font-semibold text-on-surface-variant text-left"
                    htmlFor="voice_name"
                  >
                ...
                <div className="text-[11px] text-on-surface-variant/80 mt-1 leading-relaxed text-pretty">
                  {t.rich("terms", {
                    terms: (chunks) => (
                      <a
                        href="#"
                        className="text-primary hover:text-accent-glow hover:underline transition-colors font-medium"
                      >
                        {chunks}
                      </a>
                    ),
      ...
      {/* Video Section */}
      <div className="flex-1 relative overflow-hidden order-2 min-h-[300px] lg:min-h-screen">
        <video
          autoPlay
          muted
          loop
