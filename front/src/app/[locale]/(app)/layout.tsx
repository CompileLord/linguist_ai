...
function TutorSessionsNav({
  ...
  const { data: sessions = [], refetch } = useGetTutorSessionsQuery({
    include_ended: true,
  });
  ...
  const groups: { label: string; items: TutorSessionResponse[] }[] = [
    {
      ...
      items: sessions.filter(
        (s) => new Date(s.started_at).getTime() >= todayStart,
      ),
    },
    {
      label: "Yesterday",
    ...
    },
    {
      label: "Older",
      items: sessions.filter(
        (s) => new Date(s.started_at).getTime() < yesterdayStart,
      ),
    },
  ...
  return (
    <div className="mt-0.5 ml-0.5">
      ...
      <div className="mt-0.5 max-h-52 overflow-y-auto space-y-2 pr-0.5 custom-scrollbar">
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{
                      backgroundColor: s.is_active ? "#3DD68C" : "transparent",
                      border: s.is_active
                        ? "none"
                        : "1px solid rgba(154,154,165,0.4)",
                    }}
                  />
                  <span className="truncate flex-1 leading-snug">
                    {s.title || "New Chat"}
                  </span>
                  <span className="text-[10px] text-on-surface-variant/30 shrink-0 tabular-nums">
                    {s.message_count}
                  </span>
                </button>
              );
        ...
        {sessions.length === 0 && (
          <p className="pl-9 text-[11px] text-on-surface-variant/35 italic mt-1">
            No sessions yet
          </p>
        )}
      </div>
...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  const isMissionChat =
    pathname.startsWith("/missions/") && !pathname.includes("/feedback");
  ...
  if (isOnboarding) {
    return (
      <div className="bg-background text-on-background min-h-screen flex flex-col font-body-md antialiased selection:bg-primary-container selection:text-on-primary-container overflow-x-hidden relative">
        ...
        <main className="flex-1 flex flex-col items-center justify-center  px-md mt-4 relative w-full max-w-[1400px] mx-auto z-10">
          ...
          <header className="hidden md:flex bg-surface/85 backdrop-blur-md border-b border-[#2A2A32] justify-between items-center w-full h-16 px-xl sticky top-0 z-10 shrink-0">
            ...
            <div className="flex items-center gap-md">
              ...
              <div className="flex items-center gap-xs bg-surface-bright border border-[#2A2A32] rounded-full py-1 px-3">
                <span
                  className="material-symbols-outlined text-[#E8B339] text-lg animate-pulse"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  local_fire_department
                </span>
                <span className="font-label-md text-label-md text-on-surface tabular-nums">
                  {isLoading ? "-" : gamification?.current_streak || 0} Day
                  Streak
                </span>
              </div>
              {/* XP */}
