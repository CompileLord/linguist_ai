...
import { useGetLessonByIdQuery, useGetNextLessonQuery } from "@/services/lessonApi";
...
function gradeColor(pct: number) {
  if (pct >= 90) return { text: "text-success", bg: "bg-success/[0.06]", border: "border-success/25", ring: "#3DD68C" };
  if (pct >= 70) return { text: "text-warning", bg: "bg-warning/[0.06]", border: "border-warning/25", ring: "#E8B339" };
  return { text: "text-error", bg: "bg-error/[0.06]", border: "border-error/25", ring: "#FF5C6C" };
}
function encouragement(score: number) {
  if (score >= 0.9)
    return { title: "Outstanding!", body: "You've mastered this lesson. Your hard work is paying off." };
  if (score >= 0.7)
    return { title: "Nicely done!", body: "Solid effort — a little more practice and you'll own this." };
  return { title: "Keep going!", body: "Every mistake is a step forward. Review below and try again." };
}
...
export default function CompletePage() {
  ...
  const { data: nextLesson, isLoading: nextLoading, error: nextError } =
    useGetNextLessonQuery(undefined, { skip: !fetchNext });

  // Navigate once the next lesson is ready
  useEffect(() => {
    if (fetchNext && nextLesson) router.push(`/lessons/${nextLesson.id}/theory` as any);
  }, [fetchNext, nextLesson, router]);

  if (!completionData) {
    return (
      <div className="flex flex-col items-center justify-center flex-1 gap-md py-20 text-center">
        <span className="material-symbols-outlined text-on-surface-variant text-5xl">hourglass_empty</span>
        <p className="text-on-surface-variant text-sm">Complete the quiz first to see your results.</p>
        <Button onClick={() => router.push(`/lessons/${lessonId}/theory` as any)}>Start Lesson</Button>
      </div>
    );
  }
  ...
  const mistakes = exercises
    .map((ex, i) => {
      ...
      const pct = hasOptions
        ? (norm(userAns) === norm(ex.correct_answer) ? 100 : 0)
  ...
  const stats = [
    { label: "XP Earned",      value: `+${completionData.xp_earned}`, suffix: "XP", color: "text-success",  accent: "success"  },
    { label: "Accuracy",       value: `${accuracyPct}`,                suffix: "%", color: accGrade.text,   accent: "accuracy" },
    { label: "Score",          value: `${scorePct}`,                   suffix: "%", color: scoreGrade.text,  accent: "score"    },
    { label: "Correct",        value: `${completionData.exercises_correct}`, suffix: `/${completionData.exercises_total}`, color: "text-on-surface", accent: "correct" },
  ];
      ...
      >
        ...
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div className="flex flex-col items-center gap-1.5">
            <h1 className="text-3xl font-bold text-on-surface tracking-tight">Lesson Complete!</h1>
            <p className="text-on-surface-variant/65 text-sm max-w-xs">
              You finished <span className="text-on-surface font-medium">&ldquo;{lesson?.title}&rdquo;</span> and earned your rewards.
            </p>
          </div>
          ...
          <motion.div
            ...
            className={`rounded-2xl border ${scoreGrade.border} ${scoreGrade.bg} px-4 py-3 flex items-center gap-3 max-w-sm`}
          >
            <span className={`material-symbols-outlined ${scoreGrade.text} shrink-0`} style={{ fontSize: "20px", fontVariationSettings: "'FILL' 1" }}>
              {scorePct >= 90 ? "mood" : scorePct >= 70 ? "sentiment_satisfied" : "sentiment_dissatisfied"}
            </span>
            <div className="text-left">
              <p className={`text-[14px] font-bold leading-tight ${scoreGrade.text}`}>{msg.title}</p>
              <p className="text-[12px] text-on-surface-variant/60 leading-snug mt-0.5">{msg.body}</p>
            </div>
          </motion.div>
        </div>
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl border p-4 flex flex-col gap-1.5 ${
                  s.accent === "accuracy" ? accGrade.border + " " + accGrade.bg
                  : s.accent === "score"   ? scoreGrade.border + " " + scoreGrade.bg
                  : s.accent === "success" ? "border-success/25 bg-success/[0.05]"
              ...
              >
                <span className="text-[10px] uppercase font-bold tracking-widest text-on-surface-variant/45 flex items-center gap-1.5">
                  {isXp && <span className="material-symbols-outlined text-success" style={{ fontSize: "13px" }}>bolt</span>}
                  {s.label}
                </span>
                <span className={`text-2xl font-bold tabular-nums ${s.color}`}>
                  {s.value}
                  <span className="text-sm font-semibold ml-0.5 opacity-70">{s.suffix}</span>
                </span>
              </motion.div>
            );
          })}
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-[#0E0E16] p-5 flex flex-col gap-4">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-primary/15 border border-primary/20 flex items-center justify-center">
                <span className="material-symbols-outlined text-primary" style={{ fontSize: "17px", fontVariationSettings: "'FILL' 1" }}>menu_book</span>
              </div>
              <div>
                <p className="text-[14px] font-bold text-on-surface leading-tight">What you learned</p>
                <p className="text-[11px] text-on-surface-variant/45 leading-tight mt-0.5">Key takeaways from this lesson</p>
              </div>
            </div>

            {vocab.length > 0 && (
              <div className="flex flex-col gap-2">
                <p className="text-[10px] uppercase font-bold tracking-widest text-on-surface-variant/40">Vocabulary</p>
                <div className="flex flex-wrap gap-2">
                  {vocab.slice(0, 10).map((v, i) => (
                    ...
                    >
                      <span className="text-on-surface font-semibold">{v.word}</span>
                      <span className="text-on-surface-variant/40">·</span>
                      <span className="text-on-surface-variant/60">{v.translation}</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            {(keyPoints.length > 0 || grammar) && (
              <div className="flex flex-col gap-2">
                <p className="text-[10px] uppercase font-bold tracking-widest text-on-surface-variant/40">Grammar & key points</p>
                <ul className="flex flex-col gap-1.5">
                  {keyPoints.slice(0, 5).map((p, i) => (
                    <li key={i} className="flex items-start gap-2 text-[13px] text-on-surface-variant/75 leading-relaxed">
                      <span className="material-symbols-outlined text-primary/70 shrink-0 mt-0.5" style={{ fontSize: "14px" }}>check_circle</span>
                      <span>{p}</span>
                    </li>
                  ))}
                  {grammar && (
                    <li className="flex items-start gap-2 text-[13px] text-on-surface-variant/75 leading-relaxed">
                      <span className="material-symbols-outlined text-primary/70 shrink-0 mt-0.5" style={{ fontSize: "14px" }}>school</span>
                      <span>{grammar}</span>
                    </li>
                  )}
                </ul>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              <div className="w-8 h-8 rounded-xl bg-error/15 border border-error/20 flex items-center justify-center shrink-0">
                <span className="material-symbols-outlined text-error" style={{ fontSize: "17px" }}>target</span>
              </div>
              <div className="flex-1">
                <p className="text-[13.5px] font-semibold text-on-surface leading-tight">Review your mistakes</p>
                <p className="text-[11.5px] text-on-surface-variant/45 leading-tight mt-0.5">
                  {mistakes.length} {mistakes.length === 1 ? "exercise" : "exercises"} to revisit
                </p>
              </div>
              <motion.span animate={{ rotate: showMistakes ? 180 : 0 }} className="material-symbols-outlined text-on-surface-variant/50 shrink-0" style={{ fontSize: "20px" }}>
                expand_more
              </motion.span>
            </button>
              ...
              {showMistakes && (
                ...
                >
                  {mistakes.map(({ ex, userAns, pct, answered }, i) => {
                    ...
                    return (
                      <div key={i} className={`rounded-xl border ${g.border} ${g.bg} p-3.5 flex flex-col gap-2`}>
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-[13px] text-on-surface font-medium leading-snug flex-1">{ex.question}</p>
                          <span className={`text-[10px] font-bold tabular-nums shrink-0 px-1.5 py-0.5 rounded-md ${g.text} bg-white/[0.03]`}>{pct}%</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div className="rounded-lg border border-white/[0.06] bg-[#0A0A12] p-2.5">
                            <p className="text-[9px] uppercase font-bold tracking-widest text-on-surface-variant/35 mb-1">Your answer</p>
                            <p className="text-[12.5px] text-on-surface-variant/70 leading-snug">
                              {answered ? userAns : <span className="italic text-on-surface-variant/30">skipped</span>}
                            </p>
                          </div>
                          <div className="rounded-lg border border-success/20 bg-success/[0.04] p-2.5">
                            <p className="text-[9px] uppercase font-bold tracking-widest text-success/45 mb-1">Correct</p>
                            <p className="text-[12.5px] text-success/90 font-semibold leading-snug">{ex.correct_answer}</p>
                          </div>
                        </div>
                        {ex.explanation && (
                          <p className="text-[12px] text-on-surface-variant/55 leading-relaxed flex items-start gap-1.5">
                            <span className="material-symbols-outlined text-primary/55 shrink-0 mt-0.5" style={{ fontSize: "13px" }}>school</span>
                            {ex.explanation}
                          </p>
                        )}
                      </div>
