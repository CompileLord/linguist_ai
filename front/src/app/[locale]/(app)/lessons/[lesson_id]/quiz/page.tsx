...
import { useGetLessonByIdQuery, useCompleteLessonMutation, LessonTestQuestion } from "@/services/lessonApi";
...
export default function QuizPage() {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4 py-20 text-center px-6">
        <div className="w-16 h-16 rounded-full bg-primary/15 border border-primary/30 flex items-center justify-center">
          <span className="material-symbols-outlined text-primary text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>quiz</span>
        </div>
        <p className="text-on-surface-variant text-sm">No quiz questions for this lesson.</p>
        <button
  ...
  const nextQuiz = async () => {
    ...
    if (quizIdx + 1 < quiz.length) {
      ...
      const timeSpent = Math.round((Date.now() - timeStartedRef.current) / 1000);
      try {
        ...
        >
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold uppercase tracking-widest text-primary/70 flex items-center gap-1.5">
                <span className="material-symbols-outlined" style={{ fontSize: "13px", fontVariationSettings: "'FILL' 1" }}>quiz</span>
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-[#0E0E16] p-5">
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-xl bg-primary/15 border border-primary/20 flex items-center justify-center shrink-0 mt-0.5">
                <span className="material-symbols-outlined text-primary" style={{ fontSize: "16px", fontVariationSettings: "'FILL' 1" }}>help</span>
              </div>
              <div className="flex-1">
                <p className="text-[10px] uppercase font-bold tracking-widest text-on-surface-variant/35 mb-1.5">Question</p>
          ...
          <div className="flex flex-col gap-2.5">
            {current.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => !checked && setSelected(i)} disabled={checked} className={card}>
            ...
            {checked && (
              <motion.div
                ...
                className={`rounded-2xl border p-3.5 flex items-center gap-3 ${
                  isCorrect ? "bg-success/[0.05] border-success/30" : "bg-error/[0.05] border-error/30"
                }`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                  isCorrect ? "bg-success/15" : "bg-error/15"
                }`}>
                  <span className={`material-symbols-outlined ${isCorrect ? "text-success" : "text-error"}`} style={{ fontSize: "19px", fontVariationSettings: "'FILL' 1" }}>
                    {isCorrect ? "check_circle" : "cancel"}
                  </span>
                </div>
                <div>
                  <p className={`text-[14px] font-bold leading-tight ${isCorrect ? "text-success" : "text-error"}`}>
            ...
            {!checked ? (
              ...
              >
                {isSubmitting ? (
                  ...
                  <>Next Question<span className="material-symbols-outlined" style={{ fontSize: "17px" }}>arrow_forward</span></>
                ) : (
                  <>Finish Quiz<span className="material-symbols-outlined" style={{ fontSize: "17px" }}>auto_awesome</span></>
                )}
              </button>
            )}
          </div>
