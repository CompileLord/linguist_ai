...
export default function ListeningPage() {
  ...
  const correctCount = questions.filter(
    (q, qi) => answers[qi] === q.correct_index,
  ).length;
  ...
  return (
    <motion.div
      {...slide}
      className="flex flex-col gap-lg max-w-[800px] mx-auto w-full px-sm md:px-xl py-lg"
    >
      <div>
      ...
      {script ? (
          ...
          {questions.length > 0 && (
            <div className="space-y-md">
              ...
              {questions.map((q, qi) => (
                <div
                  key={qi}
                  className="bg-[#15151A] border border-[#2A2A32] rounded-xl p-md"
                >
                  ...
                  <div className="grid grid-cols-1 gap-xs">
                    {q.options.map((opt, oi) => {
                      ...
                      let cls =
                        "flex items-center gap-sm px-sm py-xs rounded-lg border text-sm transition-all cursor-pointer ";
                      if (submitted) {
                        if (isCorrect)
                          cls +=
                            "border-emerald-500/15 bg-emerald-500/[0.06] text-emerald-500";
                        else if (isSelected)
                          cls += "border-error/50 bg-error/10 text-error";
                        else
                          cls +=
                            "border-[#2A2A32] text-on-surface-variant opacity-40";
                      } else if (isSelected)
                        cls +=
                          "border-primary/60 bg-primary/10 text-on-surface";
                      else
                        cls +=
                          "border-[#2A2A32] text-on-surface-variant hover:border-primary/40 hover:text-on-surface";
        ...
        <p className="text-on-surface-variant text-sm">
          No listening exercise for this lesson.
        </p>
      )}
      ...
      <div className="flex gap-sm">
        <Button
          variant="outline"
          onClick={() => router.push(`/lessons/${lessonId}/reading` as any)}
          className="flex-1 flex items-center justify-center gap-xs py-3"
        >
          <span className="material-symbols-outlined">arrow_back</span> Back
        </Button>
        <Button
          onClick={() => router.push(`/lessons/${lessonId}/exercises` as any)}
          className="flex-1 flex items-center justify-center gap-xs py-3"
        >
