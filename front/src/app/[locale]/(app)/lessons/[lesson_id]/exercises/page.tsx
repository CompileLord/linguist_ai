...
import { useGetLessonByIdQuery, LessonExercise } from "@/services/lessonApi";
...
function diffWords(user: string, correct: string): { u: Token[]; c: Token[] } {
  const uW = user.trim().split(/\s+/).filter(Boolean);
  const cW = correct.trim().split(/\s+/).filter(Boolean);
  return {
    u: uW.map((w, i) => ({ text: w, ok: norm(w) === norm(cW[i] ?? "") })),
    c: cW.map((w, i) => ({ text: w, ok: norm(w) === norm(uW[i] ?? "") })),
  };
}
...
function gradeSubtitle(pct: number) {
  if (pct >= 90) return "Your answer is correct.";
  if (pct >= 70) return "Almost there — check the differences below.";
  return "Review the correct answer and the grammar tip.";
}
...
export default function ExercisesPage() {
  ...
  const current: LessonExercise | undefined = exercises[exIdx];
  ...
  const nextExercise = () => {
    ...
    setChecked(false);
    setPct(0);
    if (isLast) router.push(`/lessons/${lessonId}/quiz` as any);
  };
    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Smart feedback for text answers */}
          <AnimatePresence>
            {checked && !hasOptions && (
              <motion.div
                key="text-feedback"
          ...
          {/* Grammar tip for MC */}
          <AnimatePresence>
            {checked && hasOptions && current.explanation && (
              <motion.div
                key="mc-tip"
