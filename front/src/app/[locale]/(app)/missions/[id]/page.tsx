...
import { useStartMissionMutation, useCompleteMissionMutation, useGetMissionsQuery } from "@/services/missionsApi";
...
const LANG_NAMES: Record<string, string> = {
  en: "English", ru: "Russian", tg: "Tajik", es: "Spanish", fr: "French",
  de: "German", uk: "Ukrainian", ar: "Arabic", zh: "Chinese", hi: "Hindi", tr: "Turkish",
};

const SUGGESTIONS = ["Yes, I'm ready", "Can you repeat?", "Give me vocabulary for this", "I'm not sure how to answer"];
...
function saveMemory(missionId: string, mem: MissionMemory) {
  ...
  try { localStorage.setItem(`mission_memory_${missionId}`, JSON.stringify(mem)); } catch { /* quota */ }
}
...
const MD_COMPONENTS: Components = {
  ...
  blockquote: ({ children }) => (
    <blockquote className="pl-3 my-2 text-on-surface-variant italic" style={{ borderLeft: "2px solid rgba(110,91,255,0.5)" }}>{children}</blockquote>
  ),
  h1: ({ children }) => <h1 className="text-base font-bold text-white mb-1 mt-2 first:mt-0">{children}</h1>,
  h2: ({ children }) => <h2 className="text-[15px] font-semibold text-white mb-1 mt-2 first:mt-0">{children}</h2>,
  h3: ({ children }) => <h3 className="text-sm font-semibold text-white/90 mb-1 mt-2 first:mt-0">{children}</h3>,
  ...
  th: ({ children }) => <th className="border border-[#2A2A32] px-2 py-1 text-left font-semibold text-white/90 bg-white/[0.03]">{children}</th>,
  td: ({ children }) => <td className="border border-[#2A2A32] px-2 py-1 text-on-surface/80">{children}</td>,
};
...
function AiAvatar({ accentHex, size = 32, pulse = false }: { accentHex: string; size?: number; pulse?: boolean }) {
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      {pulse && (
        <div className="absolute inset-0 rounded-full animate-ping" style={{ backgroundColor: accentHex, opacity: 0.25 }} />
      )}
      <div
      ...
      >
        <span className="material-symbols-outlined" style={{ fontSize: size * 0.48, color: accentHex, fontVariationSettings: "'FILL' 1" }}>
          smart_toy
        </span>
      </div>
    </div>
...
function ActionBar({
  isAi, accentHex, onListen, isPlaying, onTranslate, isTranslating, hasTranslation, onCopy, copied, onRegenerate,
}: {
  ...
  const btn = "flex items-center gap-1 px-2 py-1 rounded-md text-[11px] text-on-surface-variant hover:text-on-surface hover:bg-white/[0.06] transition-all active:scale-95";
  return (
    <div className={`flex items-center gap-0.5 mt-1.5 ${isAi ? "" : "justify-end"}`}>
      <button onClick={onListen} className={btn} title={isPlaying ? "Stop" : "Listen"} style={isPlaying ? { color: accentHex } : undefined}>
        <span className="material-symbols-outlined" style={{ fontSize: 14, fontVariationSettings: isPlaying ? "'FILL' 1" : "'FILL' 0" }}>
          {isPlaying ? "stop_circle" : "volume_up"}
        </span>
        <span>{isPlaying ? "Stop" : "Listen"}</span>
      </button>
      <button onClick={onTranslate} className={btn} title="Translate" disabled={isTranslating} style={hasTranslation ? { color: accentHex } : undefined}>
        <span className="material-symbols-outlined" style={{ fontSize: 14, ...(isTranslating ? { animation: "spin 1s linear infinite" } : {}), fontVariationSettings: hasTranslation ? "'FILL' 1" : "'FILL' 0" }}>
      ...
      <button onClick={onCopy} className={btn} title="Copy">
        <span className="material-symbols-outlined" style={{ fontSize: 14, fontVariationSettings: copied ? "'FILL' 1" : "'FILL' 0" }}>
...
export default function ActiveMissionPage() {
        ...
        ws.onmessage = (e) => {
          try {
            ...
            if (msg.type === "chunk") {
              ...
              streamRef.current = "";
              setStreamingText("");
              if (fullText) setMessages((prev) => [...prev, { id: newId(), role: "ai", text: fullText }]);
  ...
  const sendMessage = useCallback((text: string) => {
    ...
    if (!sendOverWs(userMsg)) {
      setTimeout(() => {
        setMessages((prev) => [...prev, { id: newId(), role: "ai", text: "Connection lost. Please refresh and try again." }]);
        setIsGenerating(false);
      }, 800);
    }
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg.type === "transcript") {
          ...
          closeSttWs();
          if (text) sendMessage(text);          // auto-send transcribed speech
    ...
    <div className="flex-1 flex flex-col overflow-hidden min-h-0 bg-[#0A0A0C]">
      ...
      <div className="shrink-0 border-b border-[#2A2A32] bg-[#15151A]/95 backdrop-blur-md">
        <div className="flex items-center gap-3 px-4 md:px-6 h-14">
          <Link href="/missions" className="flex items-center gap-1 text-on-surface-variant hover:text-on-surface text-sm transition-colors shrink-0">
          ...
          <div className="flex-1 flex items-center justify-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-on-surface-variant hidden sm:inline">· {missionTitle}</span>
                <span
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: isGenerating ? "#f97316" : "#22c55e" }} />
                <span className="text-[10px] text-on-surface-variant">
                  {isGenerating ? "Responding…" : continuous ? "Listening mode on" : "Active session"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center px-4 md:px-6 pb-3 pt-0.5 gap-0">
          {STEPS.map((step, i) => (
            <div key={step} className="flex items-center">
              {i > 0 && (
                <div className="h-px w-8 md:w-14 transition-all duration-500" style={{ backgroundColor: i <= currentStep ? accentHex : "#2A2A32" }} />
              )}
              <div className="flex items-center gap-1.5">
                ...
                >
                  {i < currentStep ? <span className="material-symbols-outlined text-[10px]">check</span> : i + 1}
                </div>
                <span className="text-[10px] font-medium hidden sm:inline transition-colors duration-300" style={{ color: i <= currentStep ? accentHex : "#9A9AA5" }}>
                  {step}
                </span>
              </div>
            </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-4 md:px-6 py-5 space-y-5">
        {messages.length === 0 && !isGenerating && (
          <div className="flex flex-col items-center justify-center h-full py-16 gap-3 text-center">
            <p className="text-sm text-on-surface-variant/60 italic max-w-md">
            ...
            {returningHint && (
              <div className="flex items-center gap-2 text-[11px] text-on-surface-variant bg-white/[0.03] border border-[#2A2A32] rounded-full px-3 py-1.5">
                <span className="material-symbols-outlined text-[14px]" style={{ color: accentHex }}>psychology</span>
            ...
            msg.role === "ai" ? (
              ...
              >
                ...
                <div className="min-w-0">
                  <span className="text-[10px] font-medium text-on-surface-variant mb-1.5 block">Elena</span>
                  <div
                  ...
                  >
                    ...
                    {translationMap[msg.id] && (
                      <div className="mt-2.5 pt-2.5 border-t border-[#2A2A32]/70">
                        <p className="text-[9px] uppercase tracking-wider font-semibold text-on-surface-variant/50 mb-1">{nativeLang}</p>
                        <p className="text-[13px] text-on-surface-variant leading-relaxed">{translationMap[msg.id]}</p>
                      </div>
                    )}
                  </div>
                  <ActionBar
                ...
                >
                  ...
                  {translationMap[msg.id] && (
                    <div className="mt-2.5 pt-2.5 border-t border-white/20">
                      <p className="text-[9px] uppercase tracking-wider font-semibold text-white/50 mb-1">{nativeLang}</p>
                      <p className="text-[13px] text-white/80 leading-relaxed">{translationMap[msg.id]}</p>
                    </div>
                  )}
                </div>
                <ActionBar
        ...
        {isGenerating && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex items-start gap-3 pr-8 md:pr-20">
            <AiAvatar accentHex={accentHex} size={32} pulse />
            <div>
              <span className="text-[10px] font-medium text-on-surface-variant mb-1.5 block">Elena</span>
              {streamingText ? (
                <div className="px-4 py-3 rounded-2xl rounded-tl-sm text-sm text-on-surface leading-relaxed border max-w-prose" style={{ backgroundColor: "#15151A", borderColor: "#2A2A32" }}>
                ...
                <div className="px-4 py-3 rounded-2xl rounded-tl-sm border" style={{ backgroundColor: "#15151A", borderColor: "#2A2A32" }}>
                  <TypingDots />
                </div>
              )}
            </div>
        ...
        <div className="flex items-center gap-2 max-w-[820px] mx-auto">
          <div className="relative flex-1">
            <input
              ...
              placeholder={isTranscribing ? "Transcribing…" : isRecording ? "Listening…" : "Type your response…"}
          ...
          <div className="relative shrink-0">
            {isRecording && <div className="absolute inset-0 rounded-xl animate-ping pointer-events-none" style={{ backgroundColor: "#ef4444", opacity: 0.2 }} />}
            <button
              ...
              style={{
                backgroundColor: isRecording ? "#ef4444" : isTranscribing ? `${accentHex}1f` : "#15151A",
                borderColor: isRecording ? "#ef4444" : "#2A2A32",
                color: isRecording ? "#fff" : isTranscribing ? accentHex : undefined,
              }}
              title={isRecording ? "Stop & send" : isTranscribing ? "Transcribing…" : "Voice input"}
            >
              <span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: isRecording ? "'FILL' 1" : "'FILL' 0" }}>
          ...
          <button
            ...
            title={continuous ? "Continuous listening on" : "Continuous listening off"}
          >
            <span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: continuous ? "'FILL' 1" : "'FILL' 0" }}>
              {continuous ? "graphic_eq" : "hearing"}
            </span>
          </button>
        </div>
        <p className="text-center text-[10px] text-on-surface-variant/40">
          Tap mic to speak (auto-sends) · toggle <span className="material-symbols-outlined text-[11px] align-middle">hearing</span> for hands-free mode
        </p>
      </div>
    </div>
  );
