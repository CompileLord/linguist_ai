    ...
    <div className="animate-fade-in pb-16">
      ...
      >
        <div
          ...
          style={{ background: "radial-gradient(ellipse 70% 90% at 100% 50%, rgba(110,91,255,0.07) 0%, transparent 65%)" }}
        />
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1 w-full">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-primary/70">Real-World Practice</span>
            </div>
            ...
            <p className="text-on-surface-variant text-sm leading-relaxed" style={{ maxWidth: "36rem" }}>
              Step into authentic scenarios with your AI conversation partner. Build confidence for restaurants, interviews, travel, and daily life.
            </p>
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="flex -space-x-3">
              {[
                { gradient: "linear-gradient(135deg, #7c3aed, #4f46e5)", icon: "face_6" },
                { gradient: "linear-gradient(135deg, #2563eb, #4338ca)", icon: "face_3" },
              ].map((av, i) => (
                <div
                  key={i}
            ...
            <div>
              <p className="text-xs font-semibold text-on-surface">AI Partners</p>
              <p className="text-[11px] text-on-surface-variant">Ready to practice</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-56 bg-surface border border-outline rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : apiError ? (
        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
          <span className="material-symbols-outlined text-error text-4xl">wifi_off</span>
          <p className="text-on-surface-variant text-sm max-w-xs">
            Could not load missions from the server. Check that the backend is running and try again.
          </p>
        </div>
      ) : items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
          <span className="material-symbols-outlined text-on-surface-variant text-4xl">explore_off</span>
          <p className="text-on-surface-variant text-sm max-w-xs">No missions available yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((m, idx) => {
            ...
            const cefr = CEFR_COLORS[m.cefr_level_min ?? ""] ?? { hex: "#6E5BFF", label: "Intermediate" };
            const goalLabel = GOAL_LABEL[m.related_goal ?? ""] ?? "Conversation";
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1.5 bg-surface-bright border border-outline rounded-full px-2.5 py-1">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-wide text-on-surface-variant">{m.cefr_level_min ?? ""}</span>
                    </div>
                  </div>
                  <div>
                    <h3 className="font-headline-md text-lg font-bold text-on-surface tracking-tight">{m.title}</h3>
                    <p className="text-sm text-on-surface-variant leading-relaxed mt-1">{m.description}</p>
                  </div>
                  <div className="mt-auto pt-3 border-t border-outline">
                    <span className="text-[11px] text-on-surface-variant">Not available</span>
                  </div>
                </motion.div>
              );
            }
            ...
            return (
              ...
              >
                <Link
                  href={`/missions/${m.id}`}
                  className="relative p-5 flex flex-col gap-3 bg-surface border border-[#2A2A32] rounded-2xl transition-all duration-300 overflow-hidden block focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 shadow-sm"
                >
                  {/* Hover glow */}
                  <div
                    className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-2xl"
                    style={{ background: `radial-gradient(ellipse 80% 60% at 50% 110%, ${cefr.hex}14 0%, transparent 65%)` }}
                  />
                  ...
                  <div className="relative z-10 flex items-start justify-between">
                    <div
                      ...
                      style={{ backgroundColor: `${cefr.hex}18`, borderColor: `${cefr.hex}50` }}
                    >
                      <span
                        className="material-symbols-outlined text-2xl"
                        style={{ color: cefr.hex, fontVariationSettings: "'FILL' 1" }}
                      >
                        {icon}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        ...
                        style={{ color: cefr.hex, backgroundColor: `${cefr.hex}12`, borderColor: `${cefr.hex}38` }}
                      >
                        {m.cefr_level_min ?? ""}
                      </span>
                    </div>
