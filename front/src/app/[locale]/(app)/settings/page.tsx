...
const SECTION = "text-[11px] font-bold text-on-surface-variant uppercase tracking-[0.08em] mb-2.5 pl-4";
const ROW = "flex items-center justify-between py-3.5 px-4 border-b border-outline last:border-0";
...
export default function SettingsPage() {
  ...
  const [lang, setLang] = useState<"en" | "ru" | "tg">(user?.ui_language ?? "en");
  ...
  return (
    <div className="animate-fade-in max-w-[800px] mx-auto pb-24 flex flex-col gap-lg">
      ...
      <div>
        ...
        <div className="bg-surface border border-outline rounded-xl shadow-md overflow-hidden">
          <div className={ROW}>
            ...
            <button className="text-xs text-primary hover:underline font-label-md">Edit</button>
          </div>
          <div className={ROW}>
            <div>
              <p className={LABEL}>Target Language</p>
              <p className={HINT}>{profile?.target_language_code?.toUpperCase() ?? "—"}</p>
            </div>
          </div>
          <div className={ROW}>
            <div>
          ...
          <div className={ROW}>
            <div>
              ...
              <p className={HINT}>{profile?.daily_goal_minutes ?? 15} minutes per day</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-surface border border-outline rounded-xl shadow-md overflow-hidden">
          <div className={ROW.replace("border-b border-outline last:border-0", "")}>
      ...
      {quota && quota.quotas.length > 0 && (
        <div>
          ...
          <div className="bg-surface border border-outline rounded-xl shadow-md overflow-hidden">
            {quota.quotas.map((q) => {
              const pct = q.daily_limit > 0 ? Math.round((q.current_usage / q.daily_limit) * 100) : 0;
              return (
                <div key={q.function_name} className={ROW}>
                  <div className="flex-1">
                    <p className={LABEL + " capitalize"}>{q.function_name.replace(/_/g, " ")}</p>
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-xs text-on-surface-variant tabular-nums">{q.current_usage}/{q.daily_limit}</span>
                    </div>
                  </div>
                </div>
              );
