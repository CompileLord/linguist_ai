...
export default function VocabularyPage() {
  ...
  const { data: userVocabResponse, isLoading: isLoadingUserVocab } =
    useGetUserVocabularyQuery({
  ...
  const { data: dictionaryResponse, isLoading: isLoadingDict } =
    useGetVocabularyListQuery(
      {
  ...
  const filteredUserItems = userVocabItems.filter((item) => {
    ...
    const matchesCefr =
      selectedCefr === "all" || vocab.cefr_level === selectedCefr;

    return matchesSearch && matchesCefr;
  });

  const displayItems =
    activeTab === "deck" ? filteredUserItems : dictionaryResponse?.items || [];
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-on-surface-variant font-body-sm">
              Search dictionary words, add custom vocab, and track your spaced
              repetition stats.
            </p>
          </div>
          <button
        ...
        <Card className="overflow-hidden bg-[#15151A] border-outline shadow-md">
          {isLoading ? (
            ...
            <div className="p-xl text-center flex flex-col items-center justify-center gap-xs">
              ...
              <p className="text-sm font-semibold text-on-surface mt-sm">
                No words found
              </p>
              <p className="text-xs text-on-surface-variant max-w-[280px]">
                Try adjusting your search criteria or add a custom word to start
                practicing.
              </p>
            </div>
          ) : (
                      ...
                      >
                        ...
                        <td className="p-4 pr-md text-right">
                          {vocab.audio_url ? (
                            <button
                              onClick={() =>
                                handlePlayAudio(vocab.audio_url, wordId)
                              }
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-[1000px] rounded-2xl border border-[#2A2A32] shadow-2xl overflow-hidden"
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
              ...
              <div className="flex gap-3 pt-2 border-t border-[#2A2A32] mt-1">
                ...
                <button
                  type="submit"
                  disabled={
                    isAdding || !newWord.trim() || !newTranslation.trim()
                  }
