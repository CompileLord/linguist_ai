...
- **Cumulative Layout Shift (CLS)**: The Dashboard relies on several independent API calls. *Solution*: Hardcode the minimum height (e.g., `min-h-[200px]`) for the widget containers and render beautiful Skeleton UI blocks. This prevents the page from aggressively jumping around as data arrives.
- **Timezone Streak Bugs**: Streaks break easily if the server assumes UTC but the user is in Asia. *Solution*: The frontend must explicitly send the `X-Timezone-Offset` header on every request, or pass the local ISO string, so the backend evaluates "midnight" correctly.
